...
Come back to this NA problem. But things worked out for `student1`.
...
which.min(student1)
```
Cool - it is the m8th element of the vector that has the lowest score. Can I remove this one?
```{r}
...
x[3]
```
Now put these bits of knowledge together to make some code that identifies and drops the lowest score (elements of the input vector) and then calculations of the mean.

```{r}
# find the lowest score 
ind <- which.min(student1)
mean(x[ -which.min(student1)])
